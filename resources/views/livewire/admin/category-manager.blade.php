<div class="space-y-8">
    <div class="flex justify-between items-center border-b border-gray-150 pb-5">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Manage global product classifications.</p>
        </div>
    </div>
            ...
            @if($deletingCategoryId)
                <div class="bg-red-50 border-b border-red-200 p-4 flex justify-between items-center animate-fade-in">
                    <div class="flex items-center gap-2 text-red-700">
                        ...
                        <span class="text-xs font-semibold">Delete this category? This cascade deletes subcategories and products.</span>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <button wire:click="$set('deletingCategoryId', null)" class="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-xs font-bold px-3 py-1.5 rounded-lg transition-all shadow-sm">
                            Cancel
                        </button>
                    </div>
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse($categories as $category)
                            <tr wire:key="{{ $category->id }}" class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $category->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-500 font-mono">{{ $category->slug }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold">
                                    <button wire:click="edit('{{ $category->id }}')" class="text-indigo-600 hover:text-indigo-950 mr-3 transition-colors">
                                        Edit
                                    </button>
                                    <button wire:click="confirmDelete('{{ $category->id }}')" class="text-red-600 hover:text-red-950 transition-colors">
                                        Delete
                                    </button>
                                </td>
                            </tr>
